...
- **[Paridad de Contenido](/wiki/Article_styling_criteria/Writing#content-parity)**: traducciones deberan contener la misma informacion que el articulo original (obviamente se esperan diferencias en la puntuación, la redacción o el formato). En lugar de cambiar el contenido de la traducción, [open an issue](https://github.com/ppy/osu-wiki/issues/new) o una `Pull Request` adicional a la original si piensas que está incompleto, inexacto o desactualizado.
- **Estructura y formato**: preview your article with a tool such as [jbt's Markdown Editor](https://jbt.github.io/markdown-editor/) to make sure it looks exactly how you wanted it to be.<!-- TODO(TicClick): we really need an article on markup features specific to the osu! wiki-->
